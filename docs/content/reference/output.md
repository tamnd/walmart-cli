...
describes what users get. Pick a format with `-o`, or let walmart choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
walmart <command> -o table   # aligned columns for reading
walmart <command> -o jsonl   # one JSON object per line, for piping
walmart <command> -o json    # a single JSON array
walmart <command> -o csv     # spreadsheet friendly
walmart <command> -o tsv     # tab-separated
walmart <command> -o url     # just the URL column
walmart <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
walmart <command> --fields id,title,url
```
...
```bash
walmart <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
walmart <command>            # a table, because this is a terminal
walmart <command> | wc -l    # JSONL, because this is a pipe
```
