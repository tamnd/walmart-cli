---
title: "Introduction"
description: "What walmart is and how it is put together."
weight: 10
---

A command line for Walmart.

walmart is a single binary. It speaks to walmart over plain HTTPS,
shapes the responses into clean records, and gets out of your way. There is
nothing to sign up for and nothing to run alongside it.

## How it is built

- A **library package** (`walmart`) holds the HTTP client and the typed
...
- A thin **`cmd/walmart`** hands the assembled app to `kit.Run`, which
...
```bash
walmart page <path>                  # the command
walmart serve --addr :7777           # GET /v1/page/<path>
walmart mcp                          # the page tool, over stdio
ant get walmart://page/<path>        # the URI dereference (via a host)
```
...
walmart is a read-only client over data walmart already serves
