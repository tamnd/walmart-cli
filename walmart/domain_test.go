...
// HTTP behaviour is covered in walmart_test.go.

func TestDomainInfo(t *testing.T) {
	info := Domain{}.Info()
	if info.Scheme != "walmart" {
		t.Errorf("Scheme = %q, want walmart", info.Scheme)
	}
	if len(info.Hosts) == 0 || info.Hosts[0] != Host {
		t.Errorf("Hosts = %v, want [%s]", info.Hosts, Host)
	}
	if info.Identity.Binary != "walmart" {
		t.Errorf("Identity.Binary = %q, want walmart", info.Identity.Binary)
	}
}
...
func TestHostWiring(t *testing.T) {
	...
	if want := "walmart://page/wiki/Go"; u.String() != want {
	...
	got, err := h.ResolveOn("walmart", "about")
	if err != nil || got.String() != "walmart://page/about" {
		t.Errorf("ResolveOn = (%q, %v), want walmart://page/about", got.String(), err)
	}
}
