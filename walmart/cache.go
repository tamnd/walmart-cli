...
// cache is a small on-disk store of responses, keyed by the request URL. Every
// surface is a different URL, so the URL is what distinguishes one response from
// another. A hit is served when the file is younger than ttl.
type cache struct {
	dir string
	ttl time.Duration
}
